    ...
    <section className={`print-invoice print-style-${(company.exportStyle ?? "CLASSIC").toLowerCase()}`}>
      ...
      <div className="print-summary">
        ...
        <div className="print-totals">
          ...
          {applied>0&&<div className="print-settlement"><span>Vypořádání záloh</span><strong>− {applied.toLocaleString("cs-CZ",{minimumFractionDigits:2})} Kč</strong></div>}{invoice.advanceApplications.length>0&&<div className="print-advances"><span className="print-label">UHRAZENÉ ZÁLOHY ZAHRNUTÉ DO VYÚČTOVÁNÍ</span>{invoice.advanceApplications.map(a=><div key={a.id} style={{display:"flex",justifyContent:"space-between",alignItems:"center",gap:"1rem",width:"100%"}}><span><strong>{a.advanceInvoice.number ?? a.advanceInvoice.id}</strong> · datum úhrady: {a.advanceInvoice.payments?.length ? a.advanceInvoice.payments.map(p=>new Date(p.paidAt).toLocaleDateString("cs-CZ")).join(", ") : "Neuvedeno"}</span><strong style={{whiteSpace:"nowrap"}}>− {Number(a.amount).toLocaleString("cs-CZ",{minimumFractionDigits:2})} Kč</strong></div>)}</div>}
