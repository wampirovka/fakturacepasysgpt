...
export default function DokladDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return <AppShell><div className="content">
    <header className="page-header">
      ...
      <div className="customer-actions print-hide"><button className="button button-primary" onClick={()=>{ const category = isCorrective ? "Opravny_doklad" : isAdvance ? "Zalohova_faktura" : "Faktura"; const originalTitle = document.title; document.title = `${category}_${invoice.number ?? "Doklad"}`; window.print(); window.setTimeout(() => { document.title = originalTitle; }, 1000); }}>Tisk / PDF</button><Link className="button button-secondary" href={isAdvance?"/zalohy":"/faktury"}>← Zpět</Link><button className="button button-secondary" onClick={()=>{ if (editing) { setEditing(false); setRepairMode(false); return; } if (locked) { if (window.confirm("Doklad je uzamčený, protože už obsahuje úhradu, vypořádanou zálohu nebo jde o opravný doklad. Chcete zapnout režim opravy? Změny se zapíšou do historie dokladu.")) { setRepairMode(true); setEditing(true); setMessage("Režim opravy je aktivní. Před uložením zkontrolujte částku a návaznosti na úhrady."); } return; } setRepairMode(false); setEditing(true); }}>{editing?"Zrušit úpravy":locked?"Opravit uzamčený doklad":"Upravit"}</button>{!locked&&<button className="button button-secondary button-delete-subtle" onClick={remove}>Smazat</button>}</div>
    </header>
    ...
    <section className={`print-invoice print-style-${(company.exportStyle ?? "CLASSIC").toLowerCase()}`}>
      ...
      <div className="print-summary">
        ...
        <div className="print-totals">
          ...
          {applied>0&&<div className="print-settlement"><span>Vypořádání záloh</span><strong>− {applied.toLocaleString("cs-CZ",{minimumFractionDigits:2})} Kč</strong></div>}{invoice.advanceApplications.length>0&&<div className="print-advances"><span className="print-label">UHRAZENÉ ZÁLOHY ZAHRNUTÉ DO VYÚČTOVÁNÍ</span><div className="print-advances-list">{invoice.advanceApplications.map(a=><div className="print-advance-row" key={a.id}><span><strong>{a.advanceInvoice.number ?? a.advanceInvoice.id}</strong></span><span>datum úhrady: {a.advanceInvoice.payments?.length ? a.advanceInvoice.payments.map(p=>new Date(p.paidAt).toLocaleDateString("cs-CZ")).join(", ") : "Neuvedeno"}</span><strong>− {Number(a.amount).toLocaleString("cs-CZ",{minimumFractionDigits:2})} Kč</strong></div>)}</div></div>}
