...
export default function ZakazniciPage(){
 const [items,setItems]=useState<Customer[]>([]),[form,setForm]=useState(empty),[edit,setEdit]=useState<string|null>(null),[q,setQ]=useState(""),[loading,setLoading]=useState(true),[saving,setSaving]=useState(false),[aresLoading,setAresLoading]=useState(false),[msg,setMsg]=useState<string|null>(null),[typeFilter,setTypeFilter]=useState("ALL");
 ...
 return <AppShell><div className="content">
  ...
  <section className="panel"><div className="panel-header"><div><h2>Seznam zákazníků</h2><span>{items.filter(c=>typeFilter==="ALL"||c.type===typeFilter).length} z {items.length} aktivních</span></div><div className="document-filters compact"><select value={typeFilter} onChange={e=>setTypeFilter(e.target.value)}><option value="ALL">Všechny typy</option><option value="BUSINESS">Firmy</option><option value="PERSON">Fyzické osoby</option></select><input className="customer-search" placeholder="Hledat název, IČO, město…" value={q} onChange={e=>setQ(e.target.value)}/></div></div>
