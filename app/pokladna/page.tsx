...
export default function PokladnaPage(){
  ...
  const filteredDocs=docs.filter(d=>{const q=search.trim().toLowerCase();return !q||[d.number??"",d.payment?.invoice.number??"",d.payment?.invoice.customer?.name??"",d.note??""].some(v=>v.toLowerCase().includes(q));});
  return <AppShell><div className="content">
    ...
    <section className="panel"><div className="panel-header"><div><h2>Pokladní doklady</h2><span>{filteredDocs.length} z {docs.length} dokladů</span></div><input className="customer-search" placeholder="Hledat doklad, fakturu nebo zákazníka…" value={search} onChange={e=>setSearch(e.target.value)}/></div>
      <div className="table-wrap"><table className="data-table"><thead><tr><th>Číslo</th><th>Datum</th><th>Faktura</th><th>Zákazník</th><th>Poznámka</th><th></th><th className="amount">Částka</th></tr></thead><tbody>{filteredDocs.length?filteredDocs.map(d=><tr key={d.id}><td>{d.payment ? <Link className="document-number-link" href={`/uhrady/${d.payment.id}`}><strong>{d.number??"-"}</strong></Link> : <strong>{d.number??"-"}</strong>}</td><td>{new Date(d.date).toLocaleDateString("cs-CZ")}</td><td>{d.payment?.invoice.number??"-"}</td><td>{d.payment?.invoice.customer?.name??"-"}</td><td>{d.note??"-"}</td><td><Link className="button button-secondary button-small" href={`/pokladna/${d.id}`}>Doklad</Link></td><td className="amount">{Number(d.amount).toLocaleString("cs-CZ",{minimumFractionDigits:2})} Kč</td></tr>):<tr><td colSpan={7} className="empty-cell">{docs.length?"Žádný doklad neodpovídá vyhledávání.":"Zatím nejsou žádné pokladní doklady."}</td></tr>}</tbody></table></div>
    </section>
