...
export async function DELETE(request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  if (!["OWNER", "ADMIN", "ACCOUNTANT"].includes(membership.role)) {
    return NextResponse.json({ error: "Nemáte oprávnění mazat úhrady." }, { status: 403 });
  }
    ...
    await prisma.$transaction(async tx => {
      ...
      const status = newPaid >= Number(payment.invoice.total) - 0.005
        ...
        : payment.invoice.dueDate && new Date(payment.invoice.dueDate).getTime() < now.getTime()
          ? (newPaid > 0.005 ? "OVERDUE" : "OVERDUE")
          : newPaid > 0.005 ? "PARTIALLY_PAID" : "ISSUED";

      if (payment.cashDocument) {
        await tx.cashDocument.delete({ where: { id: payment.cashDocument.id } });
      }
